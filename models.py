...
from abstract import ConditionalVectorField
...
class UNet(ConditionalVectorField):
    def __init__(self, in_channel: int, num_classes: int, channels: List[int], num_residual_layers: int, t_embed_dim: int, y_embed_dim: int): 
        super().__init__()
        # Initial convolution: (bs, c, h ,w) -> (bs, c_0, h, w)
        self.init_conv = nn.Sequential(nn.Conv2d(in_channel, channels[0], kernel_size=3, padding=1), nn.BatchNorm2d(channels[0]), nn.SiLU())
        ...
        # Final convolution
        self.final_conv = nn.Conv2d(channels[0], in_channel, kernel_size=3, padding=1)
